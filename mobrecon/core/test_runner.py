...
class Runner(object):
    ...
    def set_train_loader(
        self, train_loader, epochs, optimizer, scheduler, writer, board, start_epoch=0
    ):
        ...
        self.total_step = self.start_epoch * (
            len(self.train_loader.dataset) // self.writer.args.batch_size
        )
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epochs + 1):
            ...
            self.writer.save_checkpoint(
                self.model, self.optimizer, self.scheduler, self.epoch, last=True
            )
    ...
    def board_img(self, phase, n_iter, img, **kwargs):
        ...
        if kwargs.get("mask_pred") is not None:
            self.board.add_image(
                phase + "/mask_gt", tensor2array(kwargs["mask_gt"][0]), n_iter
            )
            self.board.add_image(
                phase + "/mask_pred", tensor2array(kwargs["mask_pred"][0]), n_iter
            )
        if kwargs.get("uv_pred") is not None:
            self.board.add_image(
                phase + "/uv_gt",
                tensor2array(kwargs["uv_gt"][0].sum(dim=0).clamp(max=1)),
                n_iter,
            )
            self.board.add_image(
                phase + "/uv_pred",
                tensor2array(kwargs["uv_pred"][0].sum(dim=0).clamp(max=1)),
                n_iter,
            )
        if kwargs.get("uv_prior") is not None:
            self.board.add_image(
                phase + "/uv_prior",
                tensor2array(kwargs["uv_prior"][0].sum(dim=0).clamp(max=1)),
                n_iter,
            )
    ...
    def train_a_epoch(self):
        ...
        for step, data in enumerate(self.train_loader):
            ...
            self.total_step += 1
            self.board_scalar(
                "train", self.total_step, self.optimizer.param_groups[0]["lr"], **loss
            )
            bar.suffix = (
    ...
    def evaluation(self):
            ...
            for step, data in enumerate(self.eval_loader):
                ...
                if mask_pred is not None:
                    ...
                    mask_pred = cv2.resize(
                        mask_pred, (data["img"].size(3), data["img"].size(2))
                    )
                    try:
                ...
                pred = (
                    out["mesh_pred"][0]
                    if isinstance(out["mesh_pred"], list)
                    else out["mesh_pred"]
                )
                ...
                if uv_pred.ndim == 4:
                    uv_point_pred, uv_pred_conf = map2uv(
                        uv_pred.cpu().numpy(),
                        (data["img"].size(2), data["img"].size(3)),
                    )
                else:
                ...
                bar.suffix = "({batch}/{size})".format(
                    batch=step + 1, size=len(self.eval_loader)
                )
                bar.next()
        bar.finish()
        # save to a json
        ...
        cprint(
            "Save json file at " + os.path.join(args.out_dir, args.exp_name + ".json"),
            "green",
        )
    ...
    def evaluation_withgt(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.eval_loader):
                ...
                gt = (
                    data["mesh_gt"][0]
                    if isinstance(data["mesh_gt"], list)
                    else data["mesh_gt"]
                )
                xyz_gt = data["xyz_gt"]
                pred = (
                ...
                joint_errors.append(
                    np.sqrt(np.sum((rel_joint_gt - rel_joint_pred) ** 2, axis=1))
                )
                pa_joint_errors.append(
                    np.sqrt(
                        np.sum(
                            (rel_joint_gt - rigid_align(rel_joint_pred, rel_joint_gt))
                            ** 2,
                            axis=1,
                        )
                    )
        ...
        if self.board is not None:
            self.board_scalar(
                "test", self.epoch, **{"j_loss": j_error, "pa_j_loss": pa_j_error}
            )
            self.board_img(
                "test",
                self.epoch,
    ...
    def demo(self):
        ...
        image_files = [
            os.path.join(image_fp, i) for i in os.listdir(image_fp) if "_img.jpg" in i
        ]
        ...
        with torch.no_grad():
            for step, image_path in enumerate(image_files):
                ...
                input = (
                    torch.from_numpy(base_transform(image, size=args.size))
                    .unsqueeze(0)
                    .to(self.device)
                )

                k_p = image_path.replace("_img.jpg", "_K.npy")
                ...
                save_mesh(
                    os.path.join(args.out_dir, "demo", image_name + "_mesh.ply"),
                    vertex,
                    self.faces[0],
                )

                bar.suffix = "({batch}/{size})".format(
                    batch=step + 1, size=len(image_files)
                )
                bar.next()
        bar.finish()
