...
from .network_mobrecon import DWReg2DDecode3D
...
from mobrecon.models.loss import (
    l1_loss,
    normal_loss,
    edge_length_loss,
    contrastive_loss_3d,
...
class MobRecon_DS(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.backbone = DenseStack_Backnone(
            latent_size=cfg.MODEL.LATENT_SIZE, kpts_num=cfg.MODEL.KPTS_NUM
        )
        cur_dir = os.path.dirname(os.path.realpath(__file__))
        template_fp = os.path.join(cur_dir, "../template/template.ply")
        transform_fp = os.path.join(cur_dir, "../template", "transform.pkl")
        spiral_indices, _, up_transform, tmp = spiral_tramsform(
            transform_fp,
            template_fp,
        ...
        if cfg.MODEL.DW:
            self.decoder3d = DWReg2DDecode3D(
    ...
    def loss(self, **kwargs):
        ...
        loss_dict["joint_img_loss"] = l1_loss(
            kwargs["joint_img_pred"], kwargs["joint_img_gt"]
        )
        if self.cfg.DATA.CONTRASTIVE:
            loss_dict["normal_loss"] = 0.05 * (
                normal_loss(
                    kwargs["verts_pred"][..., :3],
                    kwargs["verts_gt"][..., :3],
                    kwargs["face"],
            ...
            if kwargs["aug_param"] is not None:
                loss_dict["con3d_loss"] = contrastive_loss_3d(
                    kwargs["verts_pred"], kwargs["aug_param"]
                )
                loss_dict["con2d_loss"] = contrastive_loss_2d(
                    kwargs["joint_img_pred"], kwargs["bb2img_trans"], kwargs["size"]
                )
        else:
            loss_dict["normal_loss"] = 0.1 * normal_loss(
                kwargs["verts_pred"],
                kwargs["verts_gt"],
                kwargs["face"].to(kwargs["verts_pred"].device),
            )
            loss_dict["edge_loss"] = edge_length_loss(
                kwargs["verts_pred"],
                kwargs["verts_gt"],
                kwargs["face"].to(kwargs["verts_pred"].device),
            )
