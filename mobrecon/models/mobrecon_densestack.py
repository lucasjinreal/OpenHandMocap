...
class Backbone(nn.Module):
    def __init__(self, input_channel=128, uv_channel=21, latent_channel=256):
        ...
        self.mid_proj = conv_layer(
            1024, latent_channel, 1, 1, 0, bias=False, bn=False, relu=False
        )
        self.conv = conv_layer(uv_channel, 21, 1, bn=False, relu=False)
        self.uv_reg = nn.Sequential(
            linear_layer(latent_channel, 128, bn=False),
...
class MobRecon(nn.Module):
    ...
    def forward(self, x):
        ...
        return {"mesh_pred": pred3d, "uv_pred": pred2d_pt}
