...
def main(args):
    ...
    if args.rank == 0:
        ...
        print(args.exp_name)
    exec(
        "from mobrecon.models.{} import {}".format(
            cfg.MODEL.NAME.lower(), cfg.MODEL.NAME
        )
    )
    exec(
        "from mobrecon.datasets.{} import {}".format(
            cfg.TRAIN.DATASET.lower(), cfg.TRAIN.DATASET
        )
    )
    exec(
        "from mobrecon.datasets.{} import {}".format(
            cfg.VAL.DATASET.lower(), cfg.VAL.DATASET
        )
    )
    ...
    board = (
        SummaryWriter(args.board_dir)
        if cfg.PHASE == "train" and args.rank == 0
        else None
    )
    ...
    if cfg.PHASE in ["train", "pred"]:
        ...
        test_loader = DataLoader(
            test_dataset, batch_size=cfg.TEST.BATCH_SIZE, shuffle=False, **kwargs
        )
    else:
