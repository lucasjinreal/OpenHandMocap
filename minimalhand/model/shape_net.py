...
class ShapeNet(nn.Module):
    def __init__(self, dropout=0, _mano_root="mano_models/models"):
    ...
    def new_cal_ref_bone(self, _shape):
        ...
        shape = _shape
        th_v_shaped = (
            torch.matmul(self.mano_layer.th_shapedirs, shape.transpose(1, 0)).permute(
                2, 0, 1
            )
    ...
    def forward(self, bone_len):
        ...
        results = {"beta": beta, "bone_len_hat": bone_len_hat}
        return results
...
def save_checkpoint(
    ...
    if snapshot and state["epoch"] % snapshot == 0:
        shutil.copyfile(
            filepath,
            os.path.join(
                checkpoint, "{}_{}.pth.tar".format(fileprefix, state["epoch"])
            ),
        )

    if is_best:
        shutil.copyfile(
            filepath, os.path.join(checkpoint, "{}_best.pth.tar".format(fileprefix))
        )
